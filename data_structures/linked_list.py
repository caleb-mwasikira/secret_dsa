...
class LinkedList:
    ...
    def insert_at_middle(self, index: int, *items: int) -> bool:
        ...
        if not self.within_range(index):
            raise IndexError(
                f"Index {index} is out of bounds for linked list with range(0, {self.size() - 1})"
            )
    ...
    def search(
        self, value: int, ret_type: Union[int, Node] = int
    ) -> Optional[Union[int, Node]]:
