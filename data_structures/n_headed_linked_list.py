...
class NHeadedLinkedList:
    ...
    def merge(self, branch_name: str, to_b_name: str, on_nodes: tuple[int, int]) -> bool:
        try:
            # Get the two branches to merge together
            from_b = self.get_branch(branch_name)
            to_b = self.get_branch(to_b_name)
            from_index, to_index = on_nodes
            
            print(f"Merging {from_b.name}@index {from_index} to {to_b.name}@index {to_index}")

            if None in [from_b, to_b]:
                missing_branch: str = branch_name if from_b is None else to_b_name
                raise ValueError(f"Merge failed due to missing {missing_branch} branch")
    ...
    def __repr__(self) -> str:
        ...
        for linked_list in self.branches:
            ...
            list_str_repr.append(f"{'*' if on_current_branch else ''} {linked_list.__repr__()}")

        return "\n".join(list_str_repr)
    

def find_intersection(linked1: LinkedList, linked2: LinkedList) -> Optional[tuple[int, int]]:
